...
app.use((req, res, next) => {
  ...
  res.setHeader(
    ...
    "authorization,content-type,mcp-protocol-version,mcp-session-id,last-event-id,x-chart-output-api-key"
  );
...
function extractChartOutputApiKey(req: express.Request): string | undefined {
...
app.get("/", (_req, res) => {
  res.json({
    ...
    auth: {
      type: "bearer",
      description:
        "Pass Authorization: Bearer <CHART_OUTPUT_API_KEY>, or configure CHART_OUTPUT_API_KEY on the server.",
    },
  });
});
